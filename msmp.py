...
from utils import sameShop, diffBrand, calcSim, extract_extended_model_words, mw
import tmwm
...
def msmp(file_path, candidate_pairs_matrix, base_directory):
    ...
    for i, pi in enumerate(products):
        for j, pj in enumerate(products):
            ...
            if sameShop(pi, pj) or diffBrand(pi, pj, base_directory):
                dist[i, j] = np.inf
            else:
                ...
                pi_mw_regular, notused = extract_extended_model_words(nmki)
                pj_mw_regular, notused = extract_extended_model_words(nmkj)
